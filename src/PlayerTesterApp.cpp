...
static std::function<CYIFuture<CYIString>()> CreateMediaURLRetriever(const CYIString &issuerURL, std::function<CYIResult<CYIString>(CYIStringView)> responseParser)
{
    return [issuerURL, responseParser]() {
        // A more natural solution would be to use CYIFuture<CYIResult> but since CYIFuture does not support types
        // without default constructor, we cannot use CYIResult yet...
        // (so we try to encode our success/failure values in the same string for now)
        CYIFuture<CYIString> resultFuture;

        auto pRequest = std::shared_ptr<CYIHTTPRequest>(new CYIHTTPRequest());
        pRequest->SetURL(CYIUrl(issuerURL));
        pRequest->SetNetworkTimeoutMs(30000);
        pRequest->SetConnectionTimeoutMs(30000);
        pRequest->SetMethod(CYIHTTPRequest::Method::POST);
        pRequest->SetPostData("{}"); // We don't need to supply anything, just silence warnings about empty POST data.

        pRequest->NotifyResponse.Connect([resultFuture, responseParser](const std::shared_ptr<CYIHTTPRequest> &, const std::shared_ptr<CYIHTTPResponse> &httpResponse, bool) mutable {
            CYIString responseString = httpResponse->GetBody();
            YI_LOGD(LOG_TAG, "Received a response: [%s].", responseString.GetData());
            CYIResult<CYIString> urlResult = responseParser(responseString);
            // Since we cannot inject a CYIResult as is, use a hack: strings starting with "Error" denote errors.
            resultFuture.Set(urlResult
                                 ? urlResult.GetValue()
                                 : CYIString("Error: ") + urlResult.GetError().what());
        });

        pRequest->NotifyError.Connect([resultFuture](const std::shared_ptr<CYIHTTPRequest> &, const CYIHTTPService::HTTPStatusCode, const CYIString &errorMessage) mutable {
            resultFuture.Set(CYIString("Error: ") + errorMessage);
        });

        YI_LOGD(LOG_TAG, "Enqueuing a media URL retrieval POST request to [%s].", issuerURL.GetData());
...
static CYIResult<CYIString> ParseDynamicMediaURL(CYIStringView jsonString, const char *fieldName)
{
    CYIParsingError parsingError;
    std::unique_ptr<yi::rapidjson::Document> pDocument = CYIRapidJSONUtility::CreateDocumentFromString(jsonString, parsingError);
    if (parsingError.HasError() || !pDocument->HasMember(fieldName) || !(*pDocument)[fieldName].IsString())
    {
        return std::runtime_error(std::string("Error parsing media server response: ") + jsonString.GetRawData());
    }

    return CYIString((*pDocument)[fieldName].GetString());
}
...
PlayerTesterApp::PlayerTesterApp()
    ...
    , m_isMediaControlsHandlerSet(false)
#if defined(YI_IOS)
    , m_RoutePicker()
#endif
{
...
bool PlayerTesterApp::UserInit()
{
    ...
    CYIPushButtonView *pMediaPlaybackControlsButton = pMainComposition->GetNode<CYIPushButtonView>("MediaPlaybackControlsButton");
    YI_ASSERT(pMediaPlaybackControlsButton, LOG_TAG, "Could not find 'MediaPlaybackControlsButton'");
    ...
    CYIAbstractVideoPlayer::MediaPlaybackControlsInterface *pMediaPlaybackControlsInterface = m_pPlayer->GetMediaPlaybackControlsInterface();
    if (pMediaPlaybackControlsInterface != nullptr)
    {
        pMediaPlaybackControlsButton->SetText(pMediaPlaybackControlsInterface->IsMediaPlaybackControlEnabled() ? "Media Ctrls. On" : "Media Ctrls. Off");
        pMediaPlaybackControlsButton->ButtonPressed.Connect([this, pMediaPlaybackControlsButton]() {
            CYIAbstractVideoPlayer::MediaPlaybackControlsInterface *pMediaPlaybackControlsInterface = m_pPlayer->GetMediaPlaybackControlsInterface();
            if (pMediaPlaybackControlsInterface)
            {
                if (pMediaPlaybackControlsInterface->IsMediaPlaybackControlEnabled())
                {
                    if (m_isMediaControlsHandlerSet)
                    {
                        pMediaPlaybackControlsButton->SetText("Media Ctrls. OFF");
                        pMediaPlaybackControlsInterface->EnableMediaPlaybackControl(false);
                        pMediaPlaybackControlsInterface->SetControlsHandler(nullptr);
                        m_isMediaControlsHandlerSet = false;
                    }
                    else
                    {
                        pMediaPlaybackControlsButton->SetText("Media Ctrls. MANUAL");
                        pMediaPlaybackControlsInterface->SetControlsHandler(this);
                        m_isMediaControlsHandlerSet = true;
                    }
                }
                else
                {
                    pMediaPlaybackControlsButton->SetText("Media Ctrls. AUTO");
                    pMediaPlaybackControlsInterface->EnableMediaPlaybackControl(true);
                }
            }
        });
    }
    ...
    {
        pMediaPlaybackControlsButton->Disable();
    }
...
void PlayerTesterApp::InitializeVideoSelector(CYISceneView *pMainComposition)
{
    {
        UrlAndFormat temp;
        temp.name = "YouI HLS Live Stream";
        temp.url = "http://testbin.corp.youi.tv/live/hls/index.m3u8";
    ...
    {
        UrlAndFormat temp;
        temp.name = "YouI HLS Live Stream (360p)";
        temp.url = "http://testbin.corp.youi.tv/live/hls/360p/index.m3u8";
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::HLS;
        temp.isLive = true;
        AppendUrlIfSupported(temp);
    }
    ...
    {
        UrlAndFormat temp;
        temp.name = "YouI HLS Live Stream (720p)";
        temp.url = "http://testbin.corp.youi.tv/live/hls/720p/index.m3u8";
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::HLS;
        temp.isLive = true;
        AppendUrlIfSupported(temp);
    }
    ...
    {
        UrlAndFormat temp;
        temp.name = "YouI HLS Live Stream (1080p)";
        temp.url = "http://testbin.corp.youi.tv/live/hls/1080p/index.m3u8";
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::HLS;
        temp.isLive = true;
        AppendUrlIfSupported(temp);
    }
    ...
    {
        UrlAndFormat temp;
        temp.name = "DASH + PlayReady 2.0 (BigBuckBunny)";
        temp.url = "https://amssamples.streaming.mediaservices.windows.net/622b189f-ec39-43f2-93a2-201ac4e31ce1/BigBuckBunny.ism/manifest(format=mpd-time-csf)";
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::DASH;
    ...
    {
        UrlAndFormat temp;
        temp.name = "Metadata - ID3 PRIV";
        temp.url = "http://www.nielseninternet.com/VOD/NielsenConsumer/prog_index.m3u8";
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::HLS;
        AppendUrlIfSupported(temp);
    }
    ...
    {
        // This asset does not work on UWP because the manifest contains a field that is not supported by the UWP player.
        UrlAndFormat temp;
        temp.name = "Metadata - Inband eMsg";
        temp.url = "http://livesim.dashif.org/livesim/scte35_2/testpic_2s/Manifest.mpd";
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::DASH;
        AppendUrlIfSupported(temp);
    }
    ...
    {
        UrlAndFormat temp;
        temp.name = "Metadata - Inband eMsg (UWP)";
        temp.url = "http://se-mashup.fokus.fraunhofer.de:8080/dash/assets/adinsertion-samples/events/inband/dash.mpd";
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::DASH;
        AppendUrlIfSupported(temp);
    }
    ...
    {
        UrlAndFormat temp;
        temp.name = "Metadata - Jacob Zuma UN speech";
        temp.url = "http://aftonbladetlive-lh.akamaihd.net/i/20150928224817na_1@182509/master.m3u8?custom-mdt=on";
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::HLS;
        AppendUrlIfSupported(temp);
    }
    ...
    {
        UrlAndFormat temp;
        temp.name = "Metadata - Google SSAI";
        temp.urlRetriever = CreateMediaURLRetriever("https://dai.google.com/linear/hls/event/sN_IYUG8STe1ZzhIIE_ksA/streams",
                                                    [](CYIStringView response) {
                                                        return ParseDynamicMediaURL(response, "stream_manifest");
                                                    });
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::HLS;
        AppendUrlIfSupported(temp);
    }
    ...
    {
        UrlAndFormat temp;
        temp.name = "Audio Only";
        temp.url = "https://bitmovin-a.akamaihd.net/content/MI201109210084_1/m3u8s/f08e80da-bf1d-4e3d-8899-f0f6155f6efa-audio-only.m3u8";
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::HLS;
        AppendUrlIfSupported(temp);
    }

    {
        UrlAndFormat temp;
        temp.name = "Audio Only";
        temp.url = "https://livesim.dashif.org/dash/vod/testpic_2s/audio.mpd";
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::DASH;
        AppendUrlIfSupported(temp);
    }

    {
        UrlAndFormat temp;
        temp.name = "Audio Only";
        temp.url = "http://playready.directtaps.net/smoothstreaming/ISMAAACLC/Taxi3_AACLC.ism/Manifest";
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::Smooth;
        AppendUrlIfSupported(temp);
    }

    {
        UrlAndFormat temp;
        temp.name = "Audio Only";
        temp.url = "http://profficialsite.origin.mediaservices.windows.net/422c1c9b-933b-4ed3-a1ff-b87268cb330f/Taxi3_AACLC.mp4";
        temp.format = CYIAbstractVideoPlayer::StreamingFormat::MP4;
        AppendUrlIfSupported(temp);
    }
    ...
    for (size_t i = 0; i < items; i++)
    {
        ...
        pButtonView->Init();
        pButtonView->SetText(m_possibleURLs[i].name + " [" + FormatToString(m_possibleURLs[i].format) + "] : " + (m_possibleURLs[i].urlRetriever ? "(Dynamic URL)" : m_possibleURLs[i].url));
...
void PlayerTesterApp::ClearErrorText()
{
...
void PlayerTesterApp::OnStartButtonPressed()
{
    ...
    if (!m_pActiveFormat->urlRetriever) // The media source is static, its URL is known in advance.
    {
        PrepareVideo(m_pActiveFormat->url, *m_pActiveFormat, startTime);
    }
    else // The media stream URL is dynamic and we need to retrieve it first.
    {
        m_pStatusText->SetText("Requesting URL");

        CYIFuture<CYIString> resultFuture = m_pActiveFormat->urlRetriever();
        resultFuture.pCompleted->Connect([startTime, this](CYIString result) {
            if (result.StartsWith("Error")) // a hack until CYIFuture starts supporting CYIResult (no default constructor)
            {
                ...
                m_pErrorView->Show();
                m_pErrorView->GetNode<CYITextSceneNode>()->SetText(CYIString("Unable to retrieve media URL: ") + result.GetData());
                return;
            }

            PrepareVideo(result, *m_pActiveFormat, startTime);
        });
    }
}
...
void PlayerTesterApp::OnMinSeekButtonPressed()
{
    ...
    else
    {
        ...
        if (!ranges.empty())
        {
            ...
            m_pPlayer->Seek(smallest);
        }
        else
        {
            YI_LOGD(LOG_TAG, "GetLiveSeekableRanges() was empty. Seek not performed.");
        }
    }
}
...
void PlayerTesterApp::OnMaxSeekButtonPressed()
{
    if (!m_pPlayer->GetStatistics().isLive)
    {
        m_pPlayer->Seek(m_pPlayer->GetDurationMs() - 1000);
    }
    else
    {
        std::vector<CYIAbstractVideoPlayer::SeekableRange> ranges = m_pPlayer->GetLiveSeekableRanges();
...
void PlayerTesterApp::OnURLSelected(int32_t buttonID)
{
    ...
    m_pCurrentUrlText->SetValue(m_pActiveFormat->urlRetriever ? "(Dynamic URL)" : m_pActiveFormat->url);
...
void PlayerTesterApp::OnUserAgentButtonPressed()
{
    ...
    {
        m_userAgent = "Custom User Agent";
    }
    else
    {
        m_userAgent = "";
    }
    m_pPlayer->SetUserAgent(m_userAgent);
}
        ...
        if (type == CYIEvent::Type::KeyDown)
        {
            switch (pKeyEvent->m_keyCode)
            {
                case CYIKeyEvent::KeyCode::MediaRewind:
                    YI_LOGI(LOG_TAG, "Handling MediaRewind key event.");
                ...
                case CYIKeyEvent::KeyCode::MediaFastForward:
                    YI_LOGI(LOG_TAG, "Handling MediaFastForward key event.");
                    if (m_pPlayer->GetPlayerState() == CYIAbstractVideoPlayer::MediaState::Ready)
                    {
                ...
                case CYIKeyEvent::KeyCode::MediaStop:
                    YI_LOGI(LOG_TAG, "Handling MediaStop key event.");
                    if (m_pPlayer->GetPlayerState() == CYIAbstractVideoPlayer::MediaState::Ready)
                    {
                        OnStopButtonPressed();
                    }
                ...
                case CYIKeyEvent::KeyCode::MediaPause:
                    YI_LOGI(LOG_TAG, "Handling Pause or MediaPause key event.");
                ...
                case CYIKeyEvent::KeyCode::MediaPlay:
                    YI_LOGI(LOG_TAG, "Handling MediaPlay key event.");
                ...
                case CYIKeyEvent::KeyCode::MediaPlayPause:
                    YI_LOGI(LOG_TAG, "Handling MediaPlayPause key event.");
                    if (m_pPlayer->GetPlayerState() == CYIAbstractVideoPlayer::PlaybackState::Playing)
                    {
                        OnPauseButtonPressed();
                    }
...
void PlayerTesterApp::OnPlay(CYIAbstractVideoPlayer *pPlayer)
{
    YI_LOGI(LOG_TAG, "MediaPlaybackControls OnPlay().");
    pPlayer->Play();
}

void PlayerTesterApp::OnPause(CYIAbstractVideoPlayer *pPlayer)
{
    YI_LOGI(LOG_TAG, "MediaPlaybackControls OnPause().");
    pPlayer->Pause();
}

void PlayerTesterApp::OnSeek(CYIAbstractVideoPlayer *pPlayer, uint64_t positionMs)
{
    YI_LOGI(LOG_TAG, "MediaPlaybackControls OnSeek(%llu).", (long long unsigned)positionMs);
    pPlayer->Seek(positionMs);
}

void PlayerTesterApp::OnStop(CYIAbstractVideoPlayer *pPlayer)
{
    YI_LOGI(LOG_TAG, "MediaPlaybackControls OnStop().");
    pPlayer->Stop();
}

void PlayerTesterApp::OnClosedCaptionsEnabled(CYIAbstractVideoPlayer *pPlayer, bool enabled)
{
    YI_LOGI(LOG_TAG, "MediaPlaybackControls OnClosedCaptionsEnabled(%d).", enabled);
    pPlayer->SetClosedCaptionsEnabled(enabled);
}

void PlayerTesterApp::PrepareVideo(CYIStringView streamUrl, UrlAndFormat toPrepare, uint64_t startTime)
{
    ClearErrorText();
    ...
    {
        std::unique_ptr<CYIWidevineModularDRMConfiguration> pWidevineDRMConfiguration = std::make_unique<CYIWidevineModularDRMConfiguration>(CYIUrl("https://widevine-proxy.appspot.com/proxy"));
        pDRMConfiguration = std::move(pWidevineDRMConfiguration);
    }
    else if (toPrepare.dRMType == DRMType::WideVineBitmovinCustomRequest)
    {
        std::unique_ptr<CYIWidevineModularCustomRequestDRMConfiguration> pWidevineDRMCustomRequestConfiguration = std::make_unique<CYIWidevineModularCustomRequestDRMConfiguration>(CYIUrl("https://notagoodurl.com"));
    ...
    YI_LOGI(LOG_TAG, "Preparing media from [%s].", streamUrl.GetRawData());

    m_pPlayer->Prepare(CYIUrl(streamUrl), toPrepare.format, CYIAbstractVideoPlayer::PlaybackState::Paused, std::move(pDRMConfiguration), startTime);
}
